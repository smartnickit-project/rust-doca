...
/// and buffer information file into Memory and the user
...
pub fn save_config_info_into_buffers(
    ...
    *export_desc_len = export_desc_file_size;
    ...
    export_desc_reader.read_exact(&mut export_desc[..export_desc_file_size]).unwrap();
    ...
    *remote_addr = remote_addr_usize as *mut u8;
    ...
    buffer_info_reader.read_line(&mut remote_addr_len_buf).unwrap();
...
mod tests {
    ...
    fn bindgen_test_save_config() {
        ...
        unsafe { save_config_info_into_files(desc_string.as_bytes_mut(), src_buffer, src_buffer.len(), "/tmp/desc.txt", "/tmp/buffer.txt") };
        ...
        save_config_info_into_buffers("/tmp/desc.txt", "/tmp/buffer.txt", buffer_ref, &mut export_desc_len, &mut remote_addr, &mut remote_addr_len);
        ...
        assert_eq!(String::from_utf8(buffer_ref[..export_desc_len].to_vec()).unwrap(), String::from("Hello!"));
